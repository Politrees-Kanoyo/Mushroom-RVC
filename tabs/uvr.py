                ...
                with gr.Column(variant="panel"):
                    with gr.Group():
                        roformer_override_seg_size = gr.Checkbox(
                            ...
                            info="Переопределить размер сегмента модели по умолчанию вместо использования значения по умолчанию.",
                        )
                        with gr.Row():
                            ...
                            roformer_overlap = gr.Slider(
                                ...
                                value=8,
                                label="Перекрытие",
                                info="Количество перекрытия между окнами прогнозирования. Меньше лучше, но медленнее.",
                            )
                            roformer_pitch_shift = gr.Slider(
                                ...
                                value=0,
                                label="Сдвиг тона",
                                info="Сдвинуть тон аудио на несколько полутонов во время обработки. Может улучшить выход для глубоких/высоких вокалов.",
                            )
                with gr.Column(variant="panel"):
                    with gr.Group():
                        with gr.Row():
                            roformer_batch_size = gr.Slider(
                                ...
                                value=1,
                                label="Размер пакета",
                                info="Больший размер потребляет больше ОЗУ, но может обрабатывать немного быстрее.",
                            )
                    ...
                    with gr.Group():
                        ...
                        with gr.Row():
                            ...
                            mdx23c_overlap = gr.Slider(
                                ...
                                value=8,
                                label="Перекрытие",
                                info="Количество перекрытия между окнами прогнозирования. Больше лучше, но медленнее.",
                            )
                    ...
                    with gr.Group():
                        with gr.Row():
                            ...
                            vr_tta = gr.Checkbox(
                                value=False,
                                label="TTA",
                                info="Включить увеличение времени тестирования; медленно, но улучшает качество.",
                            )
                        ...
                        with gr.Row():
                            ...
                            vr_window_size = gr.Slider(
                                ...
                                value=512,
                                label="Размер окна",
                                info="Баланс качества и скорости. 1024 = быстро, но низкое качество, 320 = медленнее, но лучшее качество.",
                            )
                    ...
                    with gr.Group():
                        ...
                        with gr.Row():
                            demucs_seg_size = gr.Slider(
                                ...
                                value=40,
                                label="Размер сегмента",
                                info="Размер сегментов, на которые делится аудио. Больше = медленнее, но лучшее качество.",
                            )
                            demucs_overlap = gr.Slider(
                                ...
                                value=0.25,
                                label="Перекрытие",
                                info="Перекрытие между окнами прогнозирования. Больше = медленнее, но лучшее качество.",
                            )
                            demucs_shifts = gr.Slider(
                                ...
                                value=2,
                                label="Сдвиги",
                                info="Количество прогнозов со случайными сдвигами, больше = медленнее, но лучшее качество.",
                            )
                with gr.Column(variant="panel"):
                    with gr.Group():
                        with gr.Row():
        ...
        with gr.Group():
            with gr.Row():
                ...
                output_dir = gr.Textbox(
                    value="output",
                    label="Директория выходных файлов",
        ...
        with gr.Accordion("Переименовать стемы", open=False):
            ...
            with gr.Row():
                vocals_stem = gr.Textbox(
                    ...
                    info="Пример вывода: Music_(Vocals)_BS-Roformer-Viperx-1297",
                ...
                other_stem = gr.Textbox(
                    value="NAME_(STEM)_MODEL",
                    label="Другой стем",
                    info="Пример вывода: Music_(Other)_BS-Roformer-Viperx-1297",
                    placeholder="NAME_(STEM)_MODEL",
                )
            with gr.Row():
                drums_stem = gr.Textbox(
                    ...
                    info="Пример вывода: Music_(Drums)_BS-Roformer-Viperx-1297",
                    placeholder="NAME_(STEM)_MODEL",
                )
                bass_stem = gr.Textbox(
                    value="NAME_(STEM)_MODEL",
                    label="Басовый стем",
                    info="Пример вывода: Music_(Bass)_BS-Roformer-Viperx-1297",
                    placeholder="NAME_(STEM)_MODEL",
                )
            with gr.Row():
                guitar_stem = gr.Textbox(
                    value="NAME_(STEM)_MODEL",
                    label="Гитарный стем",
                    info="Пример вывода: Music_(Guitar)_BS-Roformer-Viperx-1297",
                    placeholder="NAME_(STEM)_MODEL",
                )
                piano_stem = gr.Textbox(
                    value="NAME_(STEM)_MODEL",
                    label="Пианинный стем",
                    info="Пример вывода: Music_(Piano)_BS-Roformer-Viperx-1297",
                    placeholder="NAME_(STEM)_MODEL",
                )
        ...
        with gr.Group():
            with gr.Row(equal_height=True):
                list_filter = gr.Dropdown(
                    ...
                    label="Фильтр списка",
                    info="Фильтровать и сортировать список моделей по 'стему'",
                )
